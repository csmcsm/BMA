...
/* scene data */
  ...
  Vec3fa renderPixel(const TutorialData& data, float x, float y, const ISPCCamera& camera, RayStats& stats, ExtraAddedContext& extraPart)
  {
    RandomSampler sampler;
    RandomSampler_init(sampler, (int)x, (int)y, data.accu_count);
    x += RandomSampler_get1D(sampler);
  ...
  void renderPixelStandard(const TutorialData& data,
    ...
    const unsigned int height,
    const float time,
    const ISPCCamera& camera, RayStats& stats, ExtraAddedContext& extraPart)
  {
    Vec3fa color = renderPixel(data, (float)x, (float)y, camera,stats,extraPart);
    ...
    const int numTilesY)
  {
    ...
    ExtraAddedContext extraPart(ptrArray, 0, taskIndex);
    renderPixelStandard(data, x0, y0, pixels, width, height, time, camera, g_stats[threadIndex], extraPart);
    renderPixelStandard(data, x0 + 7, y0 + 7, pixels, width, height, time, camera, g_stats[threadIndex], extraPart);
  }
  void renderTileTask_Work(int taskIndex, int threadIndex, int* pixels,
    const unsigned int width,
    const unsigned int height,
    const float time,
    const ISPCCamera& camera,
    ...
    const int numTilesY)
  {
    ...
    /////bool first = true;
    short samplePos = 0;
    ExtraAddedContext extraPart(ptrArray, 2,taskIndex);
    for (unsigned int y = y0; y < y1; y++) for (unsigned int x = x0; x < x1; x++)
    {
      samplePos++;
      if (samplePos == 1 || samplePos == 64) { continue; }
      //if (first) {
      //  first = false;
      //  continue;
      //}
      renderPixelStandard(data, x, y, pixels, width, height, time, camera, g_stats[threadIndex], extraPart);
    }
  }
    ...
    const ISPCCamera & camera)
  {
    ...
    size_t numTiles = numTilesX * numTilesY;
    parallel_for(size_t(0), numTiles, [&](const range<size_t>& range) {
      const int threadIndex = (int)TaskScheduler::threadIndex();

      for (size_t i = range.begin(); i < range.end(); i++)
        renderTileTask_Sample((int)i, threadIndex, pixels, width, height, time, camera, numTilesX, numTilesY);
      });

    parallel_for(size_t(0), size_t(numTilesX * numTilesY), [&](const range<size_t>& range) {
      //parallel_for(numTiles * i, numTiles * (i + distance), [&](const range<size_t>& range) {
      const int threadIndex = (int)TaskScheduler::threadIndex();
      for (size_t i = range.begin(); i < range.end(); i++) {
        if (ptrArray[i] == 0) {
          ...
          renderTileTask_Work((int)i, threadIndex, pixels, width, height, time, camera, numTilesX, numTilesY);
        }
      }
      });
#endif
