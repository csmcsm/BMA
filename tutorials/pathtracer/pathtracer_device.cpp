...
#define LEVEL_FACTOR    64.0f
    ...
    ExtraAddedContext* extraPart)
  {
    ...
    {
#ifdef BuddhaON
      if (extraPart->getSample == 2 && extraPart->ptrArrayContent[i] == -1)
      {
        break;
      }
#endif
      ...
      extraPart->depth = i;
      rtcInitIntersectArguments(&args, extraPart);
    ...
    ExtraAddedContext* extraPart)
  {
    ...
    for (int i = 0; i < data.spp; i++)
    {
      ...
      L = L + renderPixelFunction(data, fx, fy, sampler, camera, stats, features, extraPart);
    }
    L = L / (float)data.spp;
  ...
  //  ExtraAddedContext extraPart(0);
  //  extraPart.SetSample1000(data.max_path_length);
  //  for (int i = 0; i < data.max_path_length; i++) {
  //    extraPart.ptrArrayContent[i] = ptrArray[i][t];
  //  }
  //  for (unsigned int y=y0; y<y1; y++) for (unsigned int x=x0; x<x1; x++)
  //  {
  //    renderPixelStandard(data,x,y,pixels,width,height,time,camera,g_stats[threadIndex],RTC_FEATURE_FLAG_ALL,&extraPart);
  //  }
  //#ifdef DEBUG
  //  for (int i = 0; i < data.max_path_length; i++) {
  //    intersectorCounter[i] += extraPart.intersectCount[i];
  //  }
  //#endif // DEBUG
  //}
  void renderTileTask_Sample(int taskIndex, int threadIndex, int* pixels,
    const unsigned int width,
    const unsigned int height,
    const float time,
    const ISPCCamera& camera,
    ...
    const unsigned int x0 = tileX * TILE_SIZE_X;
    const unsigned int y0 = tileY * TILE_SIZE_Y;
    const unsigned int x1 = min(x0 + TILE_SIZE_X / 2, width);
    const unsigned int y1 = min(y0 + TILE_SIZE_Y / 2, height);
    ExtraAddedContext extraPart(0);
    extraPart.bvhRootPtr = bvhPtr;
    extraPart.getSample = 0;
    renderPixelStandard(data, x1, y1, pixels, width, height, time, camera, g_stats[threadIndex], RTC_FEATURE_FLAG_ALL, &extraPart);
    //renderPixelStandard(data, x1, y1, pixels, width, height, time, camera, g_stats[threadIndex], RTC_FEATURE_FLAG_ALL, &extraPart);


    //for (int i = 0; i < 3; i++) {
    //  for (int j = SampleDepth-1; j >= 0; j--) {
    //      ptrs[tileX][tileY][i][j] = extraPart.bvhRootPtr[i][j];
    //  }
    //}
  }
  void renderTileTask1(int taskIndex, int threadIndex, int* pixels,
    const unsigned int width,
    const unsigned int height,
    const float time,
    const ISPCCamera& camera,
    ...
    ExtraAddedContext extraPart(0);
    int count = -1;
    extraPart.ptrArrayContent = ptrs;
    for (unsigned int y = y0; y < y1; y++) for (unsigned int x = x0; x < x1; x++)
    {
      count++;
      if (count == TILE_SIZE_X * TILE_SIZE_Y / 2 + TILE_SIZE_X / 2) {
        continue;
      }
      renderPixelStandard(data, x, y, pixels, width, height, time, camera, g_stats[threadIndex], RTC_FEATURE_FLAG_ALL, &extraPart);
    }
    ...
    const ISPCCamera & camera)
  {
    ...
    if (init) {
      init = false;
      ptrs0 = new size_t * *[numTilesX];
      ptrs1 = new size_t * *[numTilesX];
      ptrs2 = new size_t * *[numTilesX];
      for (int i = 0; i < numTilesX; i++) {
        ptrs0[i] = new size_t * [Max_Path_Length];
        ptrs1[i] = new size_t * [Max_Path_Length];
        ptrs2[i] = new size_t * [Max_Path_Length];
        for (int j = 0; j < Max_Path_Length; ++j) {
          ptrs0[i][j] = new size_t[SampleDepth]{ 0 };
          ptrs1[i][j] = new size_t[SampleDepth]{ 0 };
          ptrs2[i][j] = new size_t[SampleDepth]{ 0 };
        }
      }
    }
    size_t*** largeTile0 = ptrs0;
    size_t*** largeTile1 = ptrs1;
    size_t*** largeTile2 = ptrs2;
    size_t*** tmp;
    parallel_for(0, numTilesX, [&](const range<size_t>& range) {
      const int threadIndex = (int)TaskScheduler::threadIndex();
      for (size_t i = range.begin(); i < range.end(); i++) {
        renderTileTask_Sample((int)i, threadIndex, pixels, width, height, time, camera, numTilesX, numTilesY, largeTile0[i]);
      }
      });
    parallel_for(numTilesX, numTilesX * 2, [&](const range<size_t>& range) {
      const int threadIndex = (int)TaskScheduler::threadIndex();
      for (size_t i = range.begin(); i < range.end(); i++) {
        renderTileTask_Sample((int)i, threadIndex, pixels, width, height, time, camera, numTilesX, numTilesY, largeTile1[i - numTilesX]);
      }
      });
    for (int largeTileY = 0; largeTileY < numTilesY - 2; largeTileY++) {
      size_t r = size_t((largeTileY + 2) * numTilesX);
      size_t end = size_t((largeTileY + 3) * numTilesX);
      parallel_for(r, end, [&](const range<size_t>& range) {
        const int threadIndex = (int)TaskScheduler::threadIndex();
        for (size_t i = range.begin(); i < range.end(); i++) {
          renderTileTask_Sample((int)i, threadIndex, pixels, width, height, time, camera, numTilesX, numTilesY, largeTile2[i - r]);
        }
        });
      r = size_t((largeTileY + 1) * numTilesX);
      end = size_t((largeTileY + 2) * numTilesX);

      parallel_for(size_t(r), size_t(end), [&](const range<size_t>& range) {
        const int threadIndex = (int)TaskScheduler::threadIndex();
        for (size_t i = range.begin(); i < range.end(); i++) {
          int int_i = int(i);
          if (int_i == r || int_i == end - 1) {
            size_t mappingMap[Max_Path_Length]{ 0 };
            renderTileTask1(int(i), threadIndex, pixels, width, height, time, camera, numTilesX, numTilesY, mappingMap);
            continue;
          }
          size_t mappingMap[Max_Path_Length]{ 0 };
          int i_r = (int)(i - r);
...
#ifdef BuddhaON
          map[largeTile0[i_r - 1][0][0]]++;
          map[largeTile0[i_r + 1][0][0]]++;
          map[largeTile0[i_r][0][0]]++;
          map[largeTile1[i_r - 1][0][0]]++;
          map[largeTile1[i_r + 1][0][0]]++;
          map[largeTile1[i_r][0][0]]++;
          map[largeTile2[i_r - 1][0][0]]++;
          map[largeTile2[i_r + 1][0][0]]++;
          map[largeTile2[i_r][0][0]]++;
          if (map[0] == 9)continue;
#endif // BuddhaON
            for (int s = SampleDepth - 1; s >= 0; s--) {
              if (largeTile1[i_r][0][s] == 0)continue;
              map[largeTile0[i_r - 1][0][s]]++;
              map[largeTile0[i_r + 1][0][s]]++;
              map[largeTile2[i_r - 1][0][s]]++;
              map[largeTile2[i_r + 1][0][s]]++;
              size_t b1 = largeTile1[i_r][0][s];
              if (map[b1] ==4) {
                mappingMap[0] = b1;
                break;
              }
            }
          for (int m = 1; m < Max_Path_Length; m++) {
#ifdef BuddhaON
            map.clear();
            map[largeTile0[i_r - 1][m][0]]++;
            map[largeTile0[i_r + 1][m][0]]++;
            map[largeTile2[i_r - 1][m][0]]++;
            map[largeTile2[i_r + 1][m][0]]++;
            if (map[0] == 4) {
              mappingMap[m] = -1;
              break;
...
#endif
            for (int s = SampleDepth - 1; s >= 0; s--) {
              //if (largeTile1[i - r][m][s] == 0)continue;
              map[largeTile0[i_r - 1][m][s]]++;
              map[largeTile0[i_r + 1][m][s]]++;
              map[largeTile2[i_r - 1][m][s]]++;
              map[largeTile2[i_r + 1][m][s]]++;
              size_t b1 = largeTile1[i - r][m][s];
              if (map[b1] >2) {
                  mappingMap[m] = b1;
                break;
              }
      ...
      tmp = largeTile0;
      largeTile0 = largeTile1;
      largeTile1 = largeTile2;
      largeTile2 = tmp;
    }
    parallel_for(0, numTilesX, [&](const range<size_t>& range) {
      const int threadIndex = (int)TaskScheduler::threadIndex();
    ...
    parallel_for(numTilesX * numTilesY - numTilesX, numTilesX * numTilesY, [&](const range<size_t>& range) {
      const int threadIndex = (int)TaskScheduler::threadIndex();
      for (size_t i = range.begin(); i < range.end(); i++) {
        int int_i = int(i);
        size_t mappingMap[Max_Path_Length]{ 0 };
